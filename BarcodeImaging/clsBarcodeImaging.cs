...
public class BarcodeImaging
{
	...
	//Matthias Boehnke - ERRORFIX 03
	//---------------------
	//In Code128 the minimum occurance of the narrowest bar can be 2 times lesser than in the original code! The original code
	//uses a value of 4 here, which can be wrong. An example in Code128 is the scan code for the number 30,
	//which have the following histogram:
	//|1 211232 212123 232121 2331112
	//If I mark every bar with either B (black) or W (white) we can see that there are only 2 narrowest bars:
	//|1 B2 W1 -B1- W2 B3 W2 B2 W1 B2 W1 B2 W3 B2 W3 B2 W1 B2 W1 B2 W3 B3 W1 -B1- W1 B2
	private const int MINNARROWBARCOUNT = 2; // minimum occurence of a narrow bar width
	...
	public static void ScanPage(ref System.Collections.ArrayList CodesRead, Bitmap bmp, int numscans, ScanDirection direction, BarcodeType types)
	{
		...
		for (int i = 0; i < numscans; i++)
		{
			...
			//Matthias Boehnke - ERRORFIX 04
			//---------------------
			//Pipe value in barcode causes errors in original code. 
			//Use of List<string> instead of "pipe separated string" fixed the error.
			List<string> lCodesRead = ReadBarcodes(bmp, y1, y2, direction, types);

			if ((lCodesRead != null) && (lCodesRead.Count > 0))
			{
				string[] asCodes = lCodesRead.ToArray();
	...
	public static List<string> ReadBarcodes(Bitmap bmp, int start, int end, ScanDirection direction, BarcodeType types)
	{
		//Matthias Boehnke - ERRORFIX 04
		//---------------------
		//Pipe value in barcode causes errors in original code. 
		//Use of List<string> instead of "pipe separated string" fixed the error.
		List<string> lBarCodes = new List<string>(); // will hold return values
		...
		// Return list of found barcodes, if any
		return lBarCodes;
	}
	#endregion
	...
	private static string ParseCode128(StringBuilder sbPattern)
	{
		...
		List<string> sCode128Code = new List<string>();
		uint uCheckSum = 0;

		int iCodes = 0;
		int iPos = 0;
		char cCodePage = 'B';
		bool bCodePageChanged = false;
		while (iPos <= sbPattern.Length - 6)
		{
			int iResult = ParseCode128Pattern(sbPattern.ToString(iPos, 6), sCode128Code, ref uCheckSum, ref cCodePage, ref iCodes, ref bCodePageChanged);
			switch (iResult)
			{
	...
	/// </returns>
	private static int ParseCode128Pattern(string sPattern, List<string> sResult, ref uint uCheckSum, ref char cCodePage, ref int iCodes, ref bool bCodePageChanged)
	{
		string[] Code128 = 
		...
		if ((sPattern != null) && (sPattern.Length >= 6))
		{
			for (int i = 0; i < Code128.Length; i++)
			{
				...
				{
					if (bCodePageChanged && (i != CODE128STOP))
					{
					...
					if (i == CODE128STOP)
					{
						try
						{
							//int iLength = sResult.CScanCode.Length;
							if (sResult.Count > 1)
							{
								char cCheckDigit;

								string lastItemInStringList = sResult[sResult.Count - 1];
								sResult[sResult.Count - 1] = "";
								if (cCodePage == 'C')
								{
									Int32 intCheckDigit = Convert.ToInt32(lastItemInStringList);
									cCheckDigit = (char)(intCheckDigit + 32);
								}
								else
								{
									cCheckDigit = lastItemInStringList[0];
								}
								...
								if (uCheckSum > uCheckDigit)
								{
									...
									char myTestChar = (char)((int)(uCheckSum + 32));
									if (cCheckDigit == myTestChar)
									{
										return CODE128STOP;
									}
								}
					...
					else if (uCheckSum > 0)
					{
						...
						if (cCodePage != cNewCodePage)
						{
							...
							//Matthias Boehnke - ERRORFIX 02 - Part 1
							//------------------------------
							//It can be a codePage change or it can be a checksum. We will not know about that
							//until next loop. That's why we should remember that the CodePage changes:
							bCodePageChanged = true;
						}
						switch (cCodePage)
						{
							case 'C':
								//Matthias Boehnke - ERRORFIX 01 
								//---------------------
								//i.ToString("00") can be more than 99 for the checksum. 
								//The original code used only two characters of a string to save the checksum
								//which was not enough.
								//Use of a string list instead of a string fixes the error
								sResult.Add(i.ToString("00"));
								break;
							default:
								// Regular character
								char c = (char)(i + 32);
